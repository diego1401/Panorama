...
namespace Imagine {
    ...
    inline int anyGetMouse(IntPoint2 &p, Window &win, int &subWin)
    { return anyGetMouse(p.x(), p.y(), win, subWin); }
    /// Get event.
    /// Get next event in queue. Wait for around ms milliseconds before timeout.
    /// \param ms Number of milliseconds before timeout. ms=-1: no wait limit.
